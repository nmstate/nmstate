...
pub(crate) async fn ovsdb_apply_bridges(
    ...
    for iface in merged_state.interfaces.iter() {
        ...
        if des_iface.base_iface().state == InterfaceState::Absent {
            ...
            && let Interface::OvsBridge(br_iface) = des_iface
        {
...
fn insert_bridge_options(
    row: &mut HashMap<String, serde_json::Value>,
    br_iface: &OvsBridgeInterface,
) {
    let options =
        match br_iface.bridge.as_ref().and_then(|c| c.options.as_ref()) {
            Some(o) => o,
            None => return,
        };
    if let Some(enabled) = options.stp.as_ref().and_then(|s| s.enabled) {
        row.insert("stp_enable".to_string(), serde_json::Value::Bool(enabled));
    }
    if let Some(rstp) = options.rstp {
        row.insert("rstp_enable".to_string(), serde_json::Value::Bool(rstp));
    }
    ...
    if let Some(ref fail_mode) =
        options.fail_mode.as_ref().filter(|m| !m.is_empty())
    {
        row.insert(
            "fail_mode".to_string(),
            serde_json::Value::String(fail_mode.to_string()),
        );
    }
}
...
async fn ovsdb_create_bridge(
    ...
    if let Some(port_confs) =
        br_iface.bridge.as_ref().and_then(|c| c.ports.as_ref())
    {
    ...
    insert_bridge_options(&mut bridge_row, br_iface);
...
async fn ovsdb_update_bridge(
    ...
    if let Some(ref desired_conf) = desired_br.bridge {
        insert_bridge_options(&mut bridge_row_updates, desired_br);

        // Handle port changes
        let desired_ports = desired_conf.ports.as_deref().unwrap_or(&[]);
