...
use super::{
    ...
    user::{gen_nm_user_setting, gen_user_defined_iface_setting},
...
pub(crate) fn iface_to_nm_connections(
    ...
    match iface {
        ...
        Interface::UserDefined(iface) => {
            gen_generic_setting(iface, &mut nm_conn);
            gen_user_defined_iface_setting(iface, &mut nm_conn);
        }
        _ => (),
    };
