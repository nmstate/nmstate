...
def test_edit_mac_vlan_(eth1_up):
    with macvlan_interface(
        ...
        apply_with_description(
            "Set MacVlan interface macvlan0 to up with MTU 1400", desired_state
        )
        assertlib.assert_state(desired_state)
...
def macvlan_interface(ifname, mode, promiscuous):
    ...
    try:
        apply_with_description(
            f"Configure the MacVlan interface {ifname} on the base interface "
        ...
        apply_with_description(
            f"Remove the MacVlan interface "
            f"{d_state[Interface.KEY][0][Interface.NAME]}",
            d_state,
        )
