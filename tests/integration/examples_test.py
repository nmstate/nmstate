...
def test_add_down_remove_vlan(eth1_up):
    ...
    with yaml_state('vlan101_up_creation.yml',
                    cleanup='vlan101_deletion.yml') as desired_state:
        assertlib.assert_state(desired_state)
        with yaml_state('vlan101_down_creation.yml') as desired_state:
...
def test_add_remove_ovs_bridge(eth1_up):
    with yaml_state('ovsbridge_creation.yml',
                    cleanup='ovsbridge_deletion.yml') as desired_state:
        assertlib.assert_state(desired_state)
...
def test_add_remove_linux_bridge(eth1_up):
    with yaml_state('linuxbridge_creation.yml',
                    cleanup='linuxbridge_deletion.yml') as desired_state:
        assertlib.assert_state(desired_state)
