...
from libnmstate.varlink import get_varlink_server
...
@pytest.fixture(
    params=["unix:@nmstate"], ids=["unix_socket_connection"],
)
def server(request):
    server = get_varlink_server(request.param)
...
@pytest.fixture(
    params=["unix:@nmstate"], ids=["systemd_unix_connection"],
)
...
def _format_address(server_address):
    if isinstance(server_address, tuple):
        return f"tcp:{server_address[0]}:{server_address[-1]}"
    else:
...
def test_varlink_activation_mode():
    lib_state = libnmstate.show()
    command = "varlink --activate='io.nmstate' call io.nmstate.Show"
