...
def run_server(address):
    """
    Runs the varlink server in the specified the address
    uses unix socket address and tcp address
    """
...
def validate_address(address):
    """
    Return namespaced socket address by default.
    Validates socket address if user define.
    """
    if address is None:
        return "unix:@nmstate"
...
class NmstateVarlinkService:
    ...
    def Apply(self, arguments):
        ...
        try:
            ...
            return {"result": "pass", "log": get_logs()}
...
def main():
    try:
        parser = argparse.ArgumentParser(
            description="Varlink support for libnmstate."
        )
        parser.add_argument(
            "--address",
            type=str,
            help="Unix socket address e.g: unix:/run/nmstate",
        )
        args = parser.parse_args()
        run_server(validate_address(args.address))
    except varlink.ConnectionError as exception:
