...
class State:
    ...
    def merge_interfaces(self, other_state):
        ...
        origin_other_state = State(other_state.state)
    ...
    def _sort_ovs_lag_slaves(self):
        for ifstate in self.interfaces.values():
            ports = ifstate.get(OVSBridge.CONFIG_SUBTREE, {}).get(
...
def merge_linux_bridge_ports(desired_iface_state, current_iface_state):
    ...
    if desired_ports is None and current_ports:
        desired_iface_state[LinuxBridge.CONFIG_SUBTREE].update(
            {LinuxBridge.PORT_SUBTREE: current_ports}
        )
        return
    elif not desired_ports:
        return
...
def _index_ports(ports):
    indexed_ports = {}
    for port in ports:
        port_name = port.get(LinuxBridge.Port.NAME)

        if port_name:
            indexed_ports[port_name] = port
    return indexed_ports
