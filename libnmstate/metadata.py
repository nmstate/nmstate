...
def remove_ifaces_metadata(ifaces_state):
    for iface_state in six.viewvalues(ifaces_state.interfaces):
        ...
        iface_state.get(Interface.IPV4, {}).pop(Route.KEY, None)
        iface_state.get(Interface.IPV6, {}).pop(Route.KEY, None)
...
def _generate_route_metadata(desired_state):
    """
    Save routes under interface for creating setting profile.
    ...
    for iface_name, routes in six.viewitems(desired_state.iface_routes):
        ...
        for family in (Interface.IPV4, Interface.IPV6):
            if family in iface_state:
                iface_state[family][Route.KEY] = []
            else:
                iface_state[family] = {Route.KEY: []}
        for route in routes:
            if iplib.is_ipv6_address(route[Route.DESTINATION]):
                iface_state[Interface.IPV6][Route.KEY].append(route)
            else:
                iface_state[Interface.IPV4][Route.KEY].append(route)
