...
def generate_ifaces_metadata(ctx, desired_state, current_state):
    """
    ...
    _generate_dns_metadata(ctx, desired_state, current_state)
...
def _generate_dns_metadata(ctx, desired_state, current_state):
    """
    Save DNS configuration on chosen interfaces as metadata.
    """
    _clear_current_dns_config(ctx, desired_state, current_state)
    ...
    if not servers and not searches:
        return
    if _dns_config_not_changed(ctx, desired_state, current_state):
        _preserve_current_dns_metadata(ctx, desired_state, current_state)
    else:
        ifaces_routes = {
...
def _clear_current_dns_config(ctx, desired_state, current_state):
...
def _dns_config_not_changed(ctx, desired_state, current_state):
    """
...
def _preserve_current_dns_metadata(ctx, desired_state, current_state):
    iface_dns_configs = nm.dns.get_indexed_dns_config_by_iface(
        ctx,
        nm.ipv4.acs_and_ip_profiles(ctx.client),
        nm.ipv6.acs_and_ip_profiles(ctx.client),
