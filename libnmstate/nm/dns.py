...
def get_running(ctx):
    ...
    for dns_conf in ctx.client.get_dns_configuration():
...
def get_dns_config_iface_names(
    ctx, acs_and_ipv4_profiles, acs_and_ipv6_profiles
):
    """
    ...
    for ac, ip_profile in chain(acs_and_ipv6_profiles, acs_and_ipv4_profiles):
        if ip_profile.props.dns or ip_profile.props.dns_search:
            iface_names.append(nm_ac.ActiveConnection(ctx, ac).devname)
    return iface_names
...
def get_indexed_dns_config_by_iface(
    ...
    for ac, ip_profile in acs_and_ipv6_profiles:
        if ip_profile.props.dns or ip_profile.props.dns_search:
            iface_name = nm_ac.ActiveConnection(ctx, ac).devname
