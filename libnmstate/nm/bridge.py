...
def get_info(ctx, nmdev):
    ...
    if nmdev.get_device_type() != NM.DeviceType.BRIDGE:
        return info
    bridge_setting = _get_bridge_setting(ctx, nmdev)
    if not bridge_setting:
        return info
...
def _get_bridge_setting(ctx, nmdev):
    bridge_setting = None
    bridge_con_profile = connection.ConnectionProfile(ctx)
